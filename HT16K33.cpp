...
bool HT16K33::init(uint8_t addr) {
    ...
    _buffer = static_cast<uint16_t *>(calloc(8, sizeof(uint16_t)));
 ...
 */
void HT16K33::setBrightness(uint8_t brightness) const {
 ...
 * Set the blink rate.
 */
void HT16K33::setBlink(uint8_t blink) const {
    Wire.beginTransmission(_i2c_addr);
 ...
 */
void HT16K33::writeRow(uint8_t row) const {
    // flip vertically
    if (_vFlipped) {
        row = 7 - row;
    }
